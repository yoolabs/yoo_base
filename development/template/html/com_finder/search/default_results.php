...
<ol class="search-results<?php echo $this->pageclass_sfx; ?> list-striped">
	...
	<?php foreach ($this->results as $result) :
		...
		$this->result = &$result;
		#var_dump($result);
		#var_dump($result->getTaxonomy()['Type']);
	 	#var_dump($result->getElement('element_data'));

		if(!$this->result->description) {
			$data = $result->getElement('element_data');
			if($data) {
				$this->result->description = implode("\n\n",$data);
			}
		}
